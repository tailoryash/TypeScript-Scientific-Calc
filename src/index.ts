...
let button: NodeListOf<HTMLButtonElement> = document.querySelectorAll(".btn");
...
function memoryClear(){
    let x:void|number  = localStorage.clear();
    x = 0;
...
function memoryRecall(){
    let x = localStorage.getItem(memory.toString()) as any;
...
function memoryStore() {
    ...
    result.innerHTML = mem?.toString() ??'';
    (document.getElementById('result') as HTMLElement).innerHTML = mem?.toString() ??'';

}

function trignometry() {
    let x = (document.getElementById('trigno') as HTMLInputElement).value;
    if (x == 'sin') {
        sin();
    } else if (x == 'cos') {
        cos();
...
function sin() {
    ...
    (document.getElementById("trigno") as HTMLInputElement).value ="trignometry" ;
}
...
function func() {
    let x = (document.getElementById('func') as HTMLInputElement).value;
    if (x == 'floor') {
        floor();
    } else if (x == 'ceil') {
        ceil();
...
function floor() {
    ...
    (document.getElementById("func") as HTMLInputElement).value = "function";
